...
// public domain fuzzy string match from https://github.com/forrestthewoods/lib_fts/blob/master/code/fts_fuzzy_match.h {{{
// thanks Forrest Smith
// ref: https://www.forrestthewoods.com/blog/reverse_engineering_sublime_texts_fuzzy_match/
...
void init()
{
  ...
  globalConfig().fonts.stdIconFont = atlas->AddFontFromMemoryCompressedTTF(FontAwesomeSolid_compressed_data, FontAwesomeSolid_compressed_size, 16.8f, nullptr, rangesIcons);
...
void updateContextMenu(GraphView& gv)
{
  ...
  if (ImGui::BeginPopup("Create Node")) {
    ...
    auto classList = gv.graph->getNodeClassList();
    std::multimap<int, std::string> orderedMatches; // edit distance -> string
    ...
    if (nodeClass[0] != 0) {
      for (auto const& clsname : classList) {
        int score = 0;
        if (fuzzy_match(nodeClass, clsname.c_str(), score)) {
          orderedMatches.insert({ score, clsname });
        }
      }
    } else {
      // alphabet-order
      ...
      for (auto const& clsname : classList) {
        orderedMatches.insert({ cnt, clsname });
        --cnt;
      }
    }
